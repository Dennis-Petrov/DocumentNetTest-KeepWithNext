...
const string fileName = "ТестДокнет";

var file = File.ReadAllBytes($".\\{fileName}.docx");

DocumentCore.Serial = "50024532950";

using (var ms = new MemoryStream(file))
{
    var document = DocumentCore.Load(ms, LoadOptions.DocxDefault);
    AddSignatureStamps(document);
    document.Save($"{fileName}.pdf", SaveOptions.PdfDefault);
}
...
void AddSignatureStamp(DocumentBuilder builder, Sign sign)
{
    ...
    // Probably, this should let to avoid the loop below
